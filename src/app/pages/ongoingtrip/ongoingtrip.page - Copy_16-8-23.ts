...
export class OngoingtripPage implements OnInit {
  ...
  async loadMap() {
    ...
    this.map.one(GoogleMapsEvent.MAP_READY).then((data: any) => {
      let icon: MarkerIcon = {
        url: 'https://quickcabtaxiservice.com/storage/app/public/carmarker_circle.png',
        size: {
          width: 35,
          height: 35
        }
    ...
    /*this.map.one(GoogleMapsEvent.MAP_READY).then((data: any) => {
      ...
      let coordinate_position = {
        target: coordinates,
        zoom: 16,
        tilt: 30
      };

      this.map.animateCamera(coordinate_position);
      ...
      this.marker_data = this.map.addMarkerSync(markerOptions);
      this.addmarker();
    })*/
    ...
    /*setTimeout(async () => {
      this.geolocation.watchPosition().subscribe((position) => {
          if ((position as Geoposition).coords != undefined) {
            var geoposition = (position as Geoposition);
              this.latitude = Number(geoposition.coords.latitude);
              this.longitude =  Number(geoposition.coords.longitude);
  
                this.map.clear();   
                let coordinates: LatLng = new LatLng(Number(geoposition.coords.latitude), Number(geoposition.coords.longitude));
        ...
        //this.trip_end_location_string = this.rideinfo.trip_end_location;
         // this.destination_locatoin_str = this.trip_end_location_string.split(',');
            ...
            let marker: Marker = this.map.addMarkerSync({
              ...
              'animation': 'none',
              'position': {
                'lat': Number(geoposition.coords.latitude),
                'lng': Number(geoposition.coords.longitude)
              }
            });
             ...
             /* let currentcoords = { lat: Number(geoposition.coords.latitude), lng: Number(geoposition.coords.longitude)};
              let dirservice2 = new google.maps.DirectionsService;
              let destination = {lat: Number(this.destination_locatoin_str[0]), lng: Number(this.destination_locatoin_str[1])}
  ...
  async endtrip() {
    ...
    if(this.rideinfo.tripType == 2 && this.rideinfo.current_status == 4){
      ...
      //console.log('endTrip: start ' + startpoint);
